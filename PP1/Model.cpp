...
Model::Model(const char* file, glm::vec3 pos,float scalar)
{
	// Make a JSON object
	std::string text = get_file_contents(file);
	JSON = json::parse(text);
...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix)
{
	...
	glm::vec3 translation = (glm::vec3(0.0f, 0.0f, 0.0f) + Model::pos);
	...
	glm::vec3 scale = glm::vec3(1.0f, 1.0f, 1.0f) * Model::scalar;
...
std::vector<Texture> Model::getTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		if (!skip)
		{
			...
			if (texPath.find("baseColor") || texPath.find("diffuse") != std::string::npos)
			{
			...
			else if (texPath.find("metallicRoughness") || texPath.find("specular") != std::string::npos)
			{
